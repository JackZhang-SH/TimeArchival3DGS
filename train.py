...
def _load_mask_and_roi(source_path: str, image_name: str, H: int, W: int, pad_px: int = 2):
    """
    从工作数据集的 masks_raw/<stem>.npy 加载 0/1 掩码，返回:
    (mask_hw: torch.float32 CUDA [H,W], (y0,y1,x0,x1) or None)。
    若文件不存在或全 0，返回 (None, None)。
    """
...
def _masked_l1_and_ssim(image, gt_image, mask_hw, roi, lambda_dssim, fused_ssim_available):
    """
    在 ROI 内做掩码 L1；SSIM 对掩码外像素做“完美对齐”，从而不影响相似度。
    返回 (Ll1, ssim_value) 两个标量张量。
    """
    if roi is not None:
        y0, y1, x0, x1 = roi
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    tR0 = torch.cuda.Event(enable_timing=True); tR1 = torch.cuda.Event(enable_timing=True)  # render
    tL0 = torch.cuda.Event(enable_timing=True); tL1 = torch.cuda.Event(enable_timing=True)  # loss
    tB0 = torch.cuda.Event(enable_timing=True); tB1 = torch.cuda.Event(enable_timing=True)  # backward
    tS0 = torch.cuda.Event(enable_timing=True); tS1 = torch.cuda.Event(enable_timing=True)  # optimizer step
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if getattr(opt, 'masked', False):
            ...
            if mask_hw is not None:
                Ll1, ssim_value = _masked_l1_and_ssim(image, gt_image, mask_hw, roi, opt.lambda_dssim, FUSED_SSIM_AVAILABLE)
            else:
                # 没有掩码文件时退化为全幅
        ...
        with torch.no_grad():
            ...
            # Print breakdown for A/B timing (render/loss/backward/step/iter) — now *after* step so tS0/tS1 are recorded
            if (iteration % 500 == 0):
                ...
                print(f"[TimeBreakdown] render={ms_render:.3f} ms  loss={ms_loss:.3f} ms  backward={ms_back:.3f} ms  step={ms_step:.3f} ms  iter={ms_iter:.3f} ms")
